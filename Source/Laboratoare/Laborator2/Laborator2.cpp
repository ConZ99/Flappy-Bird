...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		...
		{
			VertexFormat(glm::vec3(1, 0, 0), glm::vec3(1, 0, 2)),
			VertexFormat(glm::vec3(0.25f, 1, 0.25f), glm::vec3(1, 1, 0)),
		...
		vector<VertexFormat> patrat_vertices
		{
			VertexFormat(glm::vec3(0, 0, 0), glm::vec3(1, 0, 0)),
			VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0, 1, 0)),
		...
		vector<unsigned short> patrat_indices =
		{
			0, 1, 2,
			0, 3, 2
		};

		vector<VertexFormat> cerc_vertices{};
		vector<unsigned short> cerc_indices{};
		...
		Mesh *patrat = CreateMesh("patrat", patrat_vertices, patrat_indices);
		
		Mesh *cerc = CreateMesh("cerc", cerc_vertices, cerc_indices);
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(0, 0, 5), glm::vec3(1.0f));

	RenderMesh(meshes["tetraedru"], shaders["VertexColor"], glm::vec3(5, 0, 0), glm::vec3(2.0f));

	RenderMesh(meshes["patrat"], shaders["VertexColor"], glm::vec3(2, 2, 2), glm::vec3(2.0f));

	RenderMesh(meshes["cerc"], shaders["VertexColor"], glm::vec3(0, 0, 0), glm::vec3(2.0f));

	//RenderMesh(meshes["cerc"], shaders["VertexColor"], glm::vec3(0, -2, 0), glm::vec3(2.0f))
